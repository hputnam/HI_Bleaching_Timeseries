...
```{r}
metadata <- read.csv("~/MyProjects/HI_Bleaching_Timeseries/data/ITS2/metadata/metadata.csv", header = TRUE)
```
...
```{r}
relative.abundance.raw <- read.delim("~/MyProjects/HI_Bleaching_Timeseries/data/ITS2/processed_data/20220119T075742/its2_type_profiles/5_BleachingPairs_analysis_20220119T075742.profiles.relative.abund_and_meta.txt", sep = "\t", dec = ".", na.strings=c("","NA"))
...
ggplot(data = relative.abundance, mapping = aes(x = Bleach, y = Relative.Abundance, fill = Type.Profile)) +
...
ggplot(data = bleach, mapping = aes(x = Group, y = Relative.Abundance, fill = Type.Profile)) +
    geom_bar(position="fill", stat="identity") +
    ylab("Relative Abundance") +
    #facet_grid(~Bleach) +
    #scale_fill_manual(values = cols) +
...
ggplot(data = non.bleach, mapping = aes(x = Group, y = Relative.Abundance, fill = Type.Profile)) +
    geom_bar(position="fill", stat="identity") +
    ylab("Relative Abundance") +
    #facet_grid(~Bleach) +
    #scale_fill_manual(values = cols) +
...
ggsave("~/MyProjects/HI_Bleaching_Timeseries/data/ITS2/figures/rel.ab.ind.stitched.pdf", width = 11, height = 8, units = c("in"))
```
